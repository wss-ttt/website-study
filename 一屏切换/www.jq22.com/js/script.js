...
function Focus() {
	...
	function changePic() {
		...
		if(iActive == 0) {
			...
			doMove(bytag('ul', oBtn)[0], 'left', -(oPicLis.length - 2) * (oBtnLis[0].offsetWidth + 7));
		} else {
			doMove(bytag('ul', oBtn)[0], 'left', -(iActive - 1) * (oBtnLis[0].offsetWidth + 7));
		}
	};
...
$(function() {
	var sidebar = $('.pro-info-box'),
		mask = $('.mask'),
		close_btn = $('.close-btn'),
		click_sidebar = $('.tm-btn-clk');
...
$(function() {
	var sidebar = $('.news-info-box'),
		mask = $('.mask'),
		close_btn = $('.close-btn'),
		click_sidebar = $('.tm-btn-clk2');
